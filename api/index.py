...
# Load environment variables from .env file (local dev only — Vercel uses Dashboard env vars)
load_dotenv()

# ✅ For Vercel: api/index.py lives inside api/, so parent is project root
...
# ============================================
# ✅ For Vercel: ML models (.pkl) and car_metadata.json are in backend/ folder
ML_BASE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "backend")
ml_model = None
ml_encoders = None
...
# ✅ Vercel WSGI handler — required for @vercel/python
handler = app
